    ...
    function decodePartitions (end, vars) {
        ...
        this.word32bs('partition')
            ...
            .tap(function (vars) {
                ...
                var lastOffset = decodeMessageSet(vars.topic, vars.partition, vars.messageSet, cb);
                if (lastOffset !== -1) {
                    topics[vars.topic][vars.partition] = lastOffset;
                }
            });
    }
        ...
        Binary.parse(messageSet)
            ...
            .tap(function (vars) {
                ...
                if (!vars.partial && vars.offset !== null) {
                    messages.push(vars.value)
                    lastOffset = vars.offset;
                }
            });
        messageSet = messageSet.slice(cur);
    }
    cb && cb(null, 'message', { topic: topic, value: messages, partition: partition });
    return lastOffset;
}
